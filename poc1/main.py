...
사용법:
  ...
  # DSL 폭 지정
  python -m poc1.main --method b --width medium --width wide
"""
import argparse
import asyncio
import logging
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="POC 1: NL→DSL→Function vs NL→Function 비교")
    p.add_argument("--smoke", action="store_true",
                   help="빠른 smoke test: qwen-turbo + simple 쿼리 10개")
...
async def main() -> None:
    ...
    from poc1.config import MODELS, DSL_WIDTHS
    from poc1.dataset.queries import QUERIES, QUERIES_BY_DIFFICULTY
    from poc1.eval.runner import print_summary, run_all, save_results
